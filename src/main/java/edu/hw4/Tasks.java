...
public final class Tasks {
    ...
    public static Animal getAnimalWithTheLongestName(List<Animal> animals) {
        return animals.stream().max(Comparator.comparing(x -> x.name().length())).orElse(null);
    }

    public static Animal.Sex getSexWithBiggestQuantity(List<Animal> animals) {
        int count = (int) (animals.stream().filter(x -> x.sex() == Animal.Sex.M)
            .count() - animals.stream().filter(x -> x.sex() == Animal.Sex.F).count());
        if (count < 0) {
            return Animal.Sex.F;
        }
        return Animal.Sex.M;
    ...
    public static Map<Animal.Type, Animal> getTheHeaviestOfEachType(List<Animal> animals) {
        ...
        return animals.stream().collect(Collectors
            .toMap(Animal::type, Function.identity(), BinaryOperator.maxBy(animalComparator)));
    }

    public static Animal getTheKOldestAnimal(List<Animal> animals, int k) {
        if (animals.size() < k || k <= 0) {
            return new Animal("", null, null, 0, 0, 0, false);
        }
        return animals.stream().sorted(Comparator.comparing(Animal::age).reversed()).limit(k)
    ...
    public static Optional<Animal> theHeaviestUnderKofHeight(List<Animal> animals, int k) {
        return animals.stream().filter(x -> x.height() < k).max(Comparator.comparing(Animal::weight));
    }
    ...
    public static List<Animal> getAnimalsWhichCanBiteAndUponHundred(List<Animal> animals) {
        return animals.stream().filter(x -> x.bites() && x.height() > HUNDRED_CENTIMETERS).toList();
    }

    public static Integer getQuantityAnimalsWhichWeighIsBiggerThanHeight(List<Animal> animals) {
        return Math.toIntExact(animals.stream().filter(x -> x.weight() > x.height()).count());
    }
    ...
    public static boolean isThereADogWithHeightUponKcm(List<Animal> animals, int k) {
        return animals.stream().anyMatch(x -> x.type() == Animal.Type.DOG && x.height() > k);
    }

    public static Map<Animal.Type, Integer> getSumWeight(List<Animal> animals, int k, int l) {
        return animals.stream().filter(x -> x.age() >= k && x.age() <= l)
    ...
    public static boolean doSpidersBiteMoreOften(List<Animal> animals) {
        int spiderBitesCount = (int) animals.stream().filter(x -> x.type() == Animal.Type.SPIDER && x.bites()).count();
        int dogBitesCount = (int) animals.stream().filter(x -> x.type() == Animal.Type.DOG && x.bites()).count();
        return spiderBitesCount > dogBitesCount;
    }

    @SafeVarargs
    public static Animal findTheHeaviestFish(List<Animal>... animals1) {
        var stream = animals1[0].stream().filter(x -> x.type() == Animal.Type.FISH);
        for (int i = 1; i < animals1.length; i++) {
            stream = Stream.concat(stream, animals1[i].stream().filter(x -> x.type() == Animal.Type.FISH));
        }
        return stream.max(Comparator.comparing(Animal::weight)).orElse(null);
    }

    public static Map<String, Set<ValidationError>> getMapOfErrors(List<Animal> animals) {
        return animals.stream().filter(x -> !AnimalValidation.resultCheck(x).isEmpty())
