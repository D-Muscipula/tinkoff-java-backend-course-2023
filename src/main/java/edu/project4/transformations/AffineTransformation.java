...
public record AffineTransformation(double a, double b, double c, double d, double e, double f, Color color) implements
    Transformation {
    private static final int COLOR_BOUND = 255;

    private static boolean isAffineTransformationOK(double a, double b, double d, double e) {
        return ((a * a + d * d) < 1) && ((b * b + e * e) < 1)
            && ((a * a + b * b + d * d + e * e) < (1 + (a * e - b * d) * (a * e - b * d)));
    }

    public static AffineTransformation generateCoefficients(Random random) {
        ...
        while (!isAffineTransformationOK(a, b, d, e)) {
        ...
        return new AffineTransformation(
            a, b, c, d, e, f,
            new Color(random.nextInt(COLOR_BOUND), random.nextInt(COLOR_BOUND), random.nextInt(COLOR_BOUND))
        );
    }
    ...
    public static List<AffineTransformation> generateAffineList(int amount) {
        ...
        for (int i = 0; i < amount; i++) {
            transformations.add(AffineTransformation.generateCoefficients(random));
        }
